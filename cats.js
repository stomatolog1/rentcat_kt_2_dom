const cats = [
    ...
    {
        "name": "Жуся",
        "img_link": "photo/jusia.jpg",
        "age": 3,
        "rate": 10,
        "favourite": true,
        "description": "Немного боязливая, но при этом крайне боевая, иногда у неё появляются присту доброты и она становиться самой доброй кошкой в мире",
        "id": 9
    }
]
...
for(let i = 0; i < cats.length;i++){
    ...
    document.querySelector(".cats-box").innerHTML += `<div class="pictyres-cat" id="${i}"><h2 class="name">Имя: </h2><div class="catphoto"></div><h2 class="rate"></h2><h2 class="age">Возраст: </h2><h3 class="description"></h3><div class="favourite"></div></div>`
    if(i%2==0)document.querySelector(".cats-box").innerHTML += div;


document.getElementById(`${i}`).querySelector(".name").innerHTML +=cats[i].name

    let age = ()=>{
        if (cats[i].age == 1) return `${cats[i].age} год`
        if (cats[i].age >= 2 && cats[i].age <= 4) return `${cats[i].age} года`
        else return `${cats[i].age} лет`
        }

document.getElementById(`${i}`).querySelector(".age").innerHTML += age()
document.getElementById(`${i}`).querySelector(".rate").innerHTML = `${cats[i].rate} /10`
document.getElementById(`${i}`).querySelector(".description").innerHTML = cats[i].description

if(cats[i].favourite){document.getElementById(`${i}`).querySelector(".favourite").innerHTML = '<img src="photo/Heart_corazón.svg.webp" width="40px" height="40px"> ';}
else {document.getElementById(`${i}`).querySelector(".favourite").innerHTML = '<img src="photo/trafaret-papik-pro-dt3h-p-trafareti-pustoe-serdtse-1.jpg"  width="40px" height="40px"> '}
document.getElementById(`${i}`).querySelector(".catphoto").innerHTML = `<img src= "${cats[i].img_link}" width="420px" height="370px">`
}
